...
class ForecastDataset(torch_data.Dataset):
    ...
    # __getitem__ 方法在每次 **调用** 时自动生成两组数据：输入序列 (x) 和 目标序列 (y)
    def __getitem__(self, index):
        # 每次递增1，让每一batch的时间序列往前递增一个  0：12， 1：13，。。。
        hi = self.x_end_idx[index]
        lo = hi - self.window_size
        train_data = self.data[lo: hi]
        if hi == self.df_length:
    ...
    def __len__(self):
        return len(self.x_end_idx)  # 训练集 每组最后一个时间步长可以取的值的数量，从window_size to df length - self.horizon + 1

    def get_x_end_idx(self):
        # each element `hi` in `x_index_set` is an upper bound for get training data
        # training data range: [lo, hi), lo = hi - window_size
        if self.df_length == self.window_size:
            x_end_idx = range(self.window_size, self.df_length+1)   # 0 23
        else:
