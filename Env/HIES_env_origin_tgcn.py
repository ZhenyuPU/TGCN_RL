...
class HIES_Env:
    ...
    def __init__(self, HIES_Options, penalty_factor=10):
        ...
        self.state_dim      = 5 * self.K + 5     # 总共有12个状态分量
        self.action_dim     = 7      # 总共有7个行动分量
    ...
    def reset(self):   # 初始化日期为第1天，初始为时刻为0时刻，初始化系统状态为第1天、0时刻的状态
        self.day       =  random.randint(1, self.MAX_DAY - 2)  ### 随机选一天数据
        self.time_step = 0 
    ...
    def get_K_step(self, data, day, time_step, K):
        ...
        if day_start == day:
            ...
            start_row = data[day_start][t_start:]
            end_row = data[day][:time_step+1]
            mid_row = data[day_start+1:day]
            if mid_row.size > 0:
    ...
    def get_initial_state(self):    # 获取每一天0时刻的状态值
        ...
        state[:self.K]  =  self.get_K_step(self.lambda_b, self.day, 0, self.K)    # 电力市场买电价格
        # state[self.K:self.K*2]  =  self.get_K_step(self.lambda_s, self.day, 0, self.K)    # 电力市场卖电价格
        # state[self.K*2:self.K*3]  =  self.get_K_step(self.lambda_B, self.day, 0, self.K)    # 氢市场买氢价格

        state[self.K:self.K*2]  = self.get_K_step(self.P_solar_gen, self.day, 0, self.K)   # 光伏发电

        state[self.K*2:self.K*3]  = self.get_K_step(self.Q_ED, self.day, 0, self.K)          # 电需求
        state[self.K*3:self.K*4]  = self.get_K_step(self.Q_HD, self.day, 0, self.K)          # 热水需求
        state[self.K*4:self.K*5] = self.get_K_step(self.Q_CD, self.day, 0, self.K)          # 冷水需求

        state[-5]  = self.env_options.S_bss_init     # 初始储能能量状态
        state[-4]  = self.env_options.S_hss_init     # 初始储氢罐状态
        state[-3]  = self.env_options.S_tes_init     # 初始储热水罐状态
        state[-2]  = self.env_options.S_css_init     # 初始储冷水罐状态
    ...
    def step(self, action):                         # 执行一步决策，更新系统状态
       ...
       # 解析当前状态
        current_S_bss = self.state[-5]    # 当前储电状态
        current_S_hss = self.state[-4]    # 当前储氢量
        current_S_tes = self.state[-3]    # 当前储热水量
        current_S_css = self.state[-2]    # 当前储冷水量
       ...
       # 解析决策（保证设备运行的物理约束）
        ...
        if self.time_step < self.T:
            ...
            next_state[:self.K]  =  self.get_K_step(self.lambda_b, self.day, self.time_step + 1, self.K)    # 电力市场买电价格
            # next_state[self.K:self.K*2]  =  self.get_K_step(self.lambda_s, self.day, self.time_step + 1, self.K)    # 电力市场卖电价格
            # next_state[self.K*2:self.K*3]  =  self.get_K_step(self.lambda_B, self.day, self.time_step + 1, self.K)    # 氢市场买氢价格

            next_state[self.K:self.K*2]  = self.get_K_step(self.P_solar_gen, self.day, self.time_step + 1, self.K)   # 光伏发电

            next_state[self.K*2:self.K*3]  = self.get_K_step(self.Q_ED, self.day, self.time_step + 1, self.K)          # 电需求
            next_state[self.K*3:self.K*4]  = self.get_K_step(self.Q_HD, self.day, self.time_step + 1, self.K)          # 热水需求
            next_state[self.K*4:self.K*5] = self.get_K_step(self.Q_CD, self.day, self.time_step + 1, self.K)          # 冷水需求

            next_state[-5]  =  next_S_bss
            next_state[-4]  =  next_S_hss
            next_state[-3]  =  next_S_tes
            next_state[-2]  =  next_S_css
    ...
    def state_normalization(self, state):
        ...
        new_state[:self.K] = ( state[:self.K] - np.min(state[:self.K]) )/np.max( state[:self.K] )   # 电力市场买电价格归一化，按天归一化
        # new_state[self.K:self.K*2] = 0 #( state[1] - 0 )/np.max( self.env_options.lambda_s )   # 电力市场卖电价格归一化
        # new_state[self.K*2:self.K*3] = 0 #( state[2] - 0 )/np.max( self.env_options.lambda_B )   # 电力市场买氢价格归一化
        ...
        new_state[self.K:self.K*2]   = ( state[self.K:self.K*2] - 0 )/S_max                # 光伏发电量
        new_state[self.K*2:self.K*3] = ( state[self.K*2:self.K*3] - 0 )/S_max              # 电需求
        new_state[self.K*3:self.K*4] = ( state[self.K*3:self.K*4] - 0 )/S_max              # 热水需求
        new_state[self.K*4:self.K*5] = ( state[self.K*4:self.K*5] - 0 )/S_max              # 冷水需求


        new_state[-5]  = (state[-5] - 0)/S_max         # 储能电能量状态
        new_state[-4]  = (state[-4] - 0)/S_max         # 储氢罐储氢量
        new_state[-3]  = (state[-3] - 0)/S_max         # 储热水罐储热水量
        new_state[-2]  = (state[-2] - 0)/S_max         # 储冷水罐储冷水量
        new_state[-1]  = (state[-1] - 0)/self.T        # 当前时刻 （0~23） time_step
    ...
    def get_reward(self, action):
        ...
        P_solar_gen    = self.state[self.K*2-1]      
